...
def load_reference_data(filename):
    ...
    # add target column
    reference_data['target'] = reference_data['Appliances']    
    # reference_data = reference_data[(reference_data.target >= reference_data.target.quantile(0.1)) &
    #                                 (reference_data.target <= reference_data.target.quantile(0.9))]    
    features = ['Visibility', 'RH_5', 'Windspeed', 'RH_out']
    x_pred = dv.transform(reference_data[features].to_dict(orient='records'))
    reference_data['energy_usage'] = model.predict(x_pred)
...
def run_evidently(ref_data, data):
    ...
    mapping = ColumnMapping(prediction="energy_usage", numerical_features=['Visibility', 'RH_5', 'Windspeed', 'RH_out'],
...
def batch_analyze():
    ...
    ref_data = load_reference_data("./evidently_service/datasets/energydata_complete.csv")
    data = fetch_data()
